...
export const BettingModal = ({ market, isOpen, onClose }: BettingModalProps) => {
  ...
  const handleBet = async () => {
    ...
    setTimeout(() => {
      alert(`Bet placed! ${betAmount} GOLD on ${side.toUpperCase()}`);
      setBetting(false);
      setAmount('');
      onClose();
    }, 1500);
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fadeIn">
      <div className="bg-gray-900 border border-gold-800/40 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl shadow-gold-900/30 transform transition-all duration-300">
        <div className="sticky top-0 bg-gray-900 border-b border-gold-900/30 p-6 flex items-start justify-between">
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex items-center gap-4">
              <div className="flex-1">
                ...
                <div className="h-2 bg-gray-700 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-gold-500 to-gold-600 shadow-lg"
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                className={`p-4 rounded-lg border-2 transition-all ${
                  side === 'yes'
                    ? 'border-gold-600 bg-gold-900/20 shadow-lg shadow-gold-900/20'
          ...
          <div>
            <label className="text-sm text-gray-400 mb-3 block">Bet Amount (GOLD)</label>
            <input
              type="number"
              value={amount}
              ...
              step="0.1"
              min="0"
              className="w-full bg-gray-800 text-white text-lg px-4 py-3 rounded-lg border border-gold-900/30 focus:border-gold-700/50 focus:outline-none focus:shadow-lg focus:shadow-gold-900/10"
            />
          </div>
          ...
          {betAmount > 0 && (
            <div className="bg-gray-800/50 rounded-lg p-5 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-white font-medium">{betAmount.toFixed(2)} GOLD</span>
              </div>
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-white font-bold">{potentialReturn.toFixed(2)} GOLD</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-400">Potential Profit</span>
                <span className={`font-bold ${potentialProfit > 0 ? 'text-gold-400' : 'text-gray-400'}`}>
                  +{potentialProfit.toFixed(2)} GOLD
                </span>
              </div>
            </div>
          )}
          ...
          <button
            ...
            className={`w-full py-3.5 rounded-lg font-bold text-lg transition-all shadow-lg ${
              side === 'yes'
                ? 'bg-gradient-to-r from-gold-600 to-gold-500 hover:from-gold-500 hover:to-gold-400 text-black shadow-gold-900/30'
                : 'bg-red-700 hover:bg-red-800 text-white shadow-red-900/30'
            } disabled:opacity-50 disabled:cursor-not-allowed`}
          >
            {betting ? 'Processing...' : connected ? `Bet ${betAmount.toFixed(2)} GOLD on ${side.toUpperCase()}` : 'Connect Wallet to Bet'}
          </button>
        </div>
      </div>
    </div>
